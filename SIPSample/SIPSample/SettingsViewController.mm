...
{
    //save change
    [mPortSIPSDK clearAudioCodec];
    
    for (SettingItem  *item in settingsAudioCodec) {
        if(item.enable == YES)
        {
            [mPortSIPSDK addAudioCodec:(AUDIOCODEC_TYPE)item.codeType];
        }
    }
    
    [mPortSIPSDK clearVideoCodec];
    for (SettingItem  *item in settingsVideoCodec) {
        if(item.enable == YES)
        {
            [mPortSIPSDK addVideoCodec:(VIDEOCODEC_TYPE)item.codeType];
        }
    }
    for (SettingItem  *item in settingsAudioFeature) {
        switch (item.index) {
            case 200:
                [mPortSIPSDK enableVAD:item.enable];
                break;
            case 201:
                [mPortSIPSDK enableAEC:item.enable];
                break;
            case 202:
                [mPortSIPSDK enableANS:item.enable];
                break;
            case 203:
                [mPortSIPSDK enableAGC:item.enable];
                break;
        }
    }
